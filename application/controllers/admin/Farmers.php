...
class Farmers extends Admin_Controller {
    ...
    public function __construct()
    {
        ...
        /* Load :: Common */
        $this->lang->load('admin/farmers');

        /* Title Page :: Common */
        $this->page_title->push(lang('menu_farmers'));
        $this->data['pagetitle'] = $this->page_title->show();

        /* Breadcrumbs :: Common */
        $this->breadcrumbs->unshift(1, lang('menu_farmers'), 'admin/farmers');
    }
	...
	public function index()
	{
        ...
        else
        {
            ...
            /* Get all farmers */
            $this->data['farmers'] = $this->ion_auth->users()->result();
            foreach ($this->data['farmers'] as $k => $farmers)
            {
                $this->data['farmers'][$k]->groups = $this->ion_auth->get_users_groups($farmers->id)->result();
            }
	...
	{
        /* Breadcrumbs */
        $this->breadcrumbs->unshift(2, lang('menu_farmers_create'), 'admin/farmers/create');
        $this->data['breadcrumb'] = $this->breadcrumbs->show();
		...
		/* Validate form input */
		$this->form_validation->set_rules('name', 'lang:farmers_name', 'required');
                $this->form_validation->set_rules('idno', 'lang:farmers_idno', 'required');
	        $this->form_validation->set_rules('email', 'lang:farmers_email', 'valid_email|is_unique['.$tables['users'].'.email]');
		$this->form_validation->set_rules('phoneno', 'lang:farmers_phone', 'required');
		$this->form_validation->set_rules('company', 'lang:farmers_company', 'required');
		$this->form_validation->set_rules('password', 'lang:farmers_password', 'required|min_length[' . $this->config->item('min_password_length', 'ion_auth') . ']|max_length[' . $this->config->item('max_password_length', 'ion_auth') . ']|matches[password_confirm]');
		$this->form_validation->set_rules('password_confirm', 'lang:farmers_password_confirm', 'required');

		if ($this->form_validation->run() == TRUE)
		{
			$username = strtolower($this->input->post('name')) . ' ' . strtolower($this->input->post('last_name'));
